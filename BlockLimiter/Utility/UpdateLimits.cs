    ...
    public static class UpdateLimits
    {
        ...
        public static void Dequeue()
        {
            lock (_queue)
            {
                ...
                {
                    MyAPIGateway.Parallel.StartBackground(() => GridLimit(grid));
                    return;
                }
                ...
                if (faction != null)
                {
                    MyAPIGateway.Parallel.StartBackground(() => FactionLimit(id));
                    return;
                }

                if (!MySession.Static.Players.HasIdentity(id)) return;

                MyAPIGateway.Parallel.StartBackground(() => PlayerLimit(id));
            }
        }
            ...
            Parallel.ForEach(BlockLimiterConfig.Instance.AllLimits, new ParallelOptions { MaxDegreeOfParallelism = 5 }, limit =>
               {
                   ...
                   var limitedBlocks = playerBlocks.Count(x => limit.IsMatch(x.BlockDefinition));
                   if (limitedBlocks == 0)
                   {
                       limit.FoundEntities.Remove(id);
                       return;
