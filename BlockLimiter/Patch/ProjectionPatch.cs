...
namespace BlockLimiter.Patch
{
    ...
    public static class ProjectionPatch
    {
        ...
        private static bool PrefixInitializeClipboard(MyProjectorBase __instance)
        {
            ...
            ((IMyProjector)__instance).Enabled = false;

            // do not remove projection after it's been cleaned, just do projecto OFF/ON to update blocks.
            // overlimit Projected blocks will remain visable but as fantom, cant be welded and ignored by all tools.

            //NetworkManager.RaiseEvent(__instance, RemoveBlueprintMethod);
            //((IMyProjector)__instance).SetProjectedGrid(copiedGrid);

            ((IMyProjector)__instance).Enabled = true;
